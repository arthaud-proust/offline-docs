...
const submodules = DOCS.filter(
    (d) => selected.includes(d.name) && d.type === "submodule",
);
...
if (submodules.length) {
    ...
    execSync(`git submodule update --init --remote --merge ${paths}`, {
        cwd: ROOT,
        stdio: "inherit",
    });
}
...
if (pullPhp) {
    execSync(`node ${join(ROOT, ".mise/tasks/pull/php.js")}`, {
        cwd: ROOT,
        stdio: "inherit",
    });
}
