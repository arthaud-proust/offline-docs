      ...
      <div class="modal" role="dialog">
        ...
        <div class="modal-filters">
          <button
            ...
            class="chip" :class="{ active: !excluded.includes(src) }"
        ...
        <div class="results-area">
          ...
          <div v-else-if="query.length < 2" class="hint">Type at least 2 characters</div>
          <div v-else-if="!flat.length" class="hint">No results for "{{ query }}"</div>
          <template v-else v-for="group in grouped" :key="group.label">
            <div class="section-header">{{ group.label }}</div>
            <button
              v-for="r in group.items" :key="r.path"
              class="result-btn"
              :class="{ focused: focusedPath === r.path }"
              @click="select(r)"
              @mouseenter="focusedPath = r.path"
            >
              <span class="result-icon" :class="'icon-' + r.matchType">
                <template v-if="r.matchType === 'filename'">
                  <svg viewBox="0 0 16 16" fill="currentColor"><path d="M4 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h8a2 2 0 0 0 2-2V4.5L9.5 0H4zm0 1h5v3.5A1.5 1.5 0 0 0 10.5 6H14v8a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1z"/></svg>
                </template>
                <template v-else>#</template>
              </span>
              <span class="result-body">
                <span v-if="r.matchType === 'title'" class="result-pagename">{{ pageName(r) }}</span>
                <span class="result-title">{{ displayTitle(r) }}</span>
                <span v-if="r.matchType === 'content' && r.snippets[0]" class="result-sub">{{ r.snippets[0] }}</span>
              </span>
              <svg class="result-arrow" viewBox="0 0 16 16" fill="currentColor"><path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708z"/></svg>
            </button>
          </template>
        </div>
      </div>
...
watch(() => props.excluded, (v) => { localExcluded.value = [...(v ?? [])]; }, { deep: true });
...
watch(query, () => {
  clearTimeout(timer);
  if (query.value.length < 2) { results.value = []; loading.value = false; return; }
  loading.value = true;
...
async function doSearch() {
  ...
  const active = (props.sources ?? []).filter((s) => !localExcluded.value.includes(s));
  const params = new URLSearchParams({
    q,
    sources: active.length === (props.sources ?? []).length ? "" : active.join(","),
  });
...
function pageName(r) {
  const base = r.rel.split("/").pop().replace(/\.\w+$/, "");
...
.search-bar input {
  ...
  color: var(--text); font-size: 17px; caret-color: var(--accent);
}
...
/* Results */
.results-area { overflow-y: auto; flex: 1; padding: 8px 0 12px }
.hint { padding: 32px; text-align: center; color: var(--muted); font-size: 15px }
...
.result-btn.focused, .result-btn:hover { background: var(--bg3) }
...
.result-icon.icon-filename { color: var(--muted) }
.result-icon.icon-title, .result-icon.icon-content { color: var(--accent) }

.result-body { flex: 1; display: flex; flex-direction: column; gap: 1px; min-width: 0 }
.result-pagename {
...
.result-arrow { width: 14px; height: 14px; color: var(--muted); flex-shrink: 0 }
</style>
